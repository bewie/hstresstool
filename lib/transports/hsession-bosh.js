...
hSessionBosh.prototype = {
	...
	suscribe: function() {
		...
		/*var iqid2 = Control.connection.pubsub.subscribe(Control.connection.jid,
			Config.PUBSUB_COMPONENT,
			Config.NODE_NAME_WEEK,
			[],
			Client.on_event_week,
			Client.on_subscribe_week);
			  
	        
			//console.log("baro : " + Client.subscribed_to_baro);
			//console.log("week : " + Client.subscribed_to_week);
		*/
	...
	onMessage: function(message) {
		...
		{ 
			...
			if (_data) {
				var myData = jQuery.parseJSON( _data );
				if( myData.alert.topic == Config.NODE_NAME_BARO ){
					Config.DATA_BARO = myData;
					//console.log('baro data: ' + myData.alert.messages[0]);
				}
			}
		}*/
		return true;
